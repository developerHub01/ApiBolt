...
import { HttpStatusUpdatePayloadInterface } from "@/shared/types/http-status.type.js";
import { WithNullable } from "@/shared/types/index.js";
...
export const getHttpStatusByCode = async (code: string) => {
  try {
    return (
      await db
        .select()
...
export const updateHttpStatus = async (
  payload: WithNullable<
    HttpStatusUpdatePayloadInterface,
    "editedDescription" | "editedReason"
  >
) => {
  try {

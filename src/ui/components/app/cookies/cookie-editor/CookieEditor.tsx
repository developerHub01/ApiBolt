...
const nonEditableField = new Set(["creation", "lastAccessed"]);
...
const CookieEditor = memo(({ details, onChange, bottomAction }: Props) => {
  return (
    <div className="flex flex-col gap-3" key={"add-cookie"}>
      <Table className="w-full border table-fixed overflow-hidden">
        ...
        <TableBody>
          {(
            Object.keys(DEFAULT_COOKIE_DETAILS) as Array<keyof CookieInterface>
          ).map((key) => {

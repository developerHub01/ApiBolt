...
${formData.map(({ key, value, type }) => (type === "text" ? `\t\t{${JSON.stringify(key)}, ${JSON.stringify(value)}}` : `\t\t{:file, "/path/your_path_here", {"form-data", [name: ${JSON.stringify(key + "[]")}, filename: ${JSON.stringify(value)}]}, []}`)).join(",\n")}
\t]}`;
  }

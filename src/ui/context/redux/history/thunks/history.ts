...
>("history/replaceCurrentByHistory", async (_, { dispatch, getState }) => {
  try {
     ...
     */
    if (headers?.length) {
      const headersResponse = await window.electronAPIHeadersDB.replaceHeaders(
        requestId,
     ...
     */
    {
      const paramsResponse = await window.electronAPIParamsDB.replaceParams(
        requestId,
        params
      );
      if (paramsResponse)
    ...
    if (body?.type) {
      const bodyTypeResponse =
        await window.electronAPIRequestMetaTabDB.replaceRequestMetaTab({
    ...
    if (body?.type === "form-data") {
      const formDataResponse =
        await window.electronAPIBodyFormDataDB.replaceFullBodyFormData(
          requestId,
    ...
    if (body?.type === "x-www-form-urlencoded") {
      const xWWWFormUrlencodedResponse =
        await window.electronAPIBodyXWWWFormUrlencodedDB.replaceBodyXWWWFormUrlencoded(
          requestId,
    ...
    if (body?.type === "binary") {
      const binaryResponse =
        await window.electronAPIBodyBinaryDB.replaceBodyBinary({
          requestOrFolderMetaId: requestId,
     ...
     */
    if (body?.type === "raw") {
      const rawResponse = await window.electronAPIBodyRawDB.replaceBodyRaw({
        requestOrFolderMetaId: requestId,
        rawData: body.raw,
        type: body.rawType,
      });

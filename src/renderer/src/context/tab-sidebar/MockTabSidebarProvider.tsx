...
import {
  ...
  loadTabsData,
  removeTab,
} from "@/context/redux/mock/thunks/tab-list";
import {
  selectIsTabListCollapsed,
  selectSelectedTab,
  selectTabList,
} from "@/context/redux/mock/selectors/tab-list";
...
interface MockTabSidebarContext {
...
const MockTabSidebarContext = createContext<MockTabSidebarContext | null>(null);

export const useMockTabSidebar = () => {
  const context = useContext(MockTabSidebarContext);

  if (!context) {
    throw new Error(
      "useMockTabSidebar must be used within a MockTabSidebarProvider.",
    );
  }
...
interface MockTabSidebarProviderProps {
  children: React.ReactNode;
}

const MockTabSidebarProvider = ({ children }: MockTabSidebarProviderProps) => {
  ...
  return (
    <MockTabSidebarContext.Provider
      value={{
        selectedTab,
        tabList,
        isCollapsed,
...
export default MockTabSidebarProvider;

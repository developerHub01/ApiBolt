...
>("project/changeActiveProject", async (id, { dispatch, getState }) => {
  try {
    ...
    const response =
      await window.electronAPIProjects.changeActiveProject(newActiveProjectId);
...
>("project/deleteProject", async (id, { dispatch }) => {
  try {
    const response = await window.electronAPIProjects.deleteProjects(id);
...
>("project/exportProject", async (id, { dispatch }) => {
  try {
    const response = await window.electronAPIProjects.exportProject(id);
    if (response) dispatch(loadProjectList());
...
>("project/importProject", async (_, { dispatch }) => {
  try {
    const response = await window.electronAPIProjects.importProject();

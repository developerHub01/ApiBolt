...
export const selectIsHttpMethodType = createSelector(
  [
    (state: RootState) =>
      state.mock.requestList[state.mock.selectedTab!]?.method ?? "get",
  ],
...
export const selectRequestName = createSelector(
  [
    (state: RootState) =>
      state.mock.requestList[state.mock.selectedTab!]?.name ?? "Request",
  ],
  (name): string => name,
);

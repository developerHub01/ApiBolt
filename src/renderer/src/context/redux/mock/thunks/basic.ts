...
import { handleKeepOnlyWindowedRequests } from "@/context/redux/mock/mock-slice";
...
>("mock/keepOnlyWindowedRequests", async (_, { dispatch, getState }) => {
  ...
  dispatch(
    handleKeepOnlyWindowedRequests(state.requestResponseWindow.requestTimeLine),
  );
});

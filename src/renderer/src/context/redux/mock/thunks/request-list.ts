...
import { loadTabsData } from "@/context/redux/mock/thunks/tab-list";
...
>("mock/forceLoadRequestList", async (_, { dispatch }) => {
  try {
    const list =
      await window.electronAPIMockRequestOrFolderMeta.getMockRequestOrFolderMeta();
...
>("mock/createSingleRequest", async (parentId, { getState, dispatch }) => {
  try {
    const state = getState() as RootState;
...
>("mock/createCollection", async (parentId, { getState, dispatch }) => {
  try {
    const state = getState() as RootState;
    const projectId = state.project.activeProjectId;
    if (!projectId) throw new Error();
...
>("mock/createRestApiBasic", async (parentId, { getState, dispatch }) => {
  try {
    const state = getState() as RootState;
    const projectId = state.project.activeProjectId;
    if (!projectId) throw new Error();
...
>(
  "mock/createSingleRequestBySelectedTab",
...
>("mock/createCollectionBySelectedTab", async (_, { dispatch, getState }) => {
  try {
    ...
    if (response) await dispatch(expendRequestOrFolder(parentId ?? undefined));
    return response;
  } catch (error) {
    console.error(error);
    return false;
...
>("mock/updateRequestOrFolder", async (payload, { dispatch }) => {
  try {
    dispatch(handleUpdateRequestOrFolder(payload));
    await window.electronAPIMockRequestOrFolderMeta.updateMockRequestOrFolderMeta(
      payload,
    );
  } catch (error) {
    console.error(error);
...
>("mock/expendRequestOrFolder", async (id, { dispatch, getState }) => {
  try {
    const state = getState() as RootState;
...
>(
  ...
  async ({ requestId, parentId = null }, { dispatch, getState }) => {
    try {
      ...
      const response =
        await window.electronAPIMockRequestOrFolderMeta.moveMockRequestOrFolderMeta(
          {
            id: requestId,
            parentId,
          },
...
>("mock/deleteAllRequestOrFolder", async (id, { dispatch }) => {
  try {
    dispatch(handleDeleteAllRequestOrFolder());
    await window.electronAPIMockRequestOrFolderMeta.deleteMockRequestOrFolderMetaByProjectId(
      id,
    );
  } catch (error) {
    console.error(error);
...
>("mock/deleteRequestOrFolder", async (payload, { getState, dispatch }) => {
  try {
    const state = getState() as RootState;
    ...
    const response =
      await window.electronAPIMockRequestOrFolderMeta.deleteMockRequestOrFolderMetaById(
        idsToDelete,
      );
    if (response) {
      dispatch(handleChangeIsRequestListLoaded(false));
...
>(
  ...
  async (id: string, { dispatch, getState }) => {
    try {
      ...
      const response =
        await window.electronAPIMockRequestOrFolderMeta.duplicateMockRequestOrFolderMeta(
          duplicatedDataFiltered,
        );

      if (!response) return false;

      await window.electronAPIRequest.duplicateRequestOrFolderByOldNewIds(
        oldNewIdMap,
      );
      // await Promise.all([
      //   /* duplicate urls */
...
>("mock/collapseAllRequestOrFolder", async (_, { dispatch }) => {
  try {
    const response =
      await window.electronAPIMockRequestOrFolderMeta.collapseAllMockRequestOrFolderMeta();

...
>(
  ...
  async (payload, { dispatch, getState }) => {
    try {
      ...
      const response =
        await window.electronAPIBodyBinary.getBodyBinary(selectedTab);
...
>("request-response/updateRequestBodyBinary", async (_, { dispatch }) => {
  try {
    const response = await window.electronAPIBodyBinary.updateBodyBinary();
...
>("request-response/deleteRequestBodyBinary", async (_, { dispatch }) => {
  try {
    const response = await window.electronAPIBodyBinary.deleteBodyBinary();
...
>("request-response/duplicateBodyBinaryByOldNewIds", async oldNewIdMap => {
  try {
    const response =
      await window.electronAPIBodyBinary.duplicateBodyBinary(oldNewIdMap);

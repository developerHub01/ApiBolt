...
>("request-response/loadParams", async (payload, { getState, dispatch }) => {
  try {
    ...
    const response = await window.electronAPIParams.getParams(selectedTab);
...
>("request-response/deleteParams", async (id, { getState, dispatch }) => {
  try {
    ...
    const response = await window.electronAPIParams.deleteParams(id);
...
>(
  ...
  async (id, { getState, dispatch }) => {
    try {
      ...
      const response =
        await window.electronAPIParams.deleteParamsByRequestMetaId(id);
...
>("request-response/duplicateParamsByOldNewIds", async oldNewIdMap => {
  try {
    const response =
      await window.electronAPIParams.duplicateParams(oldNewIdMap);

...
>("request-response/addHeaders", async (payload, { getState, dispatch }) => {
  try {
    ...
    if (!payload) payload = {};
    const response = await window.electronAPIHeaders.createHeaders(payload);
...
>("request-response/deleteHeaders", async (id, { getState, dispatch }) => {
  try {
    ...
    const response = await window.electronAPIHeaders.deleteHeaders(id);
...
>("request-response/duplicateHeadersByOldNewIds", async oldNewIdMap => {
  try {
    const response =
      await window.electronAPIHeaders.duplicateHeaders(oldNewIdMap);

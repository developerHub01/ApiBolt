...
import {
  selectCookieTrackingGlobal,
  selectCookieTrackingLocal,
...
const SettingCookieTracking = () => {
  const dispatch = useAppDispatch();
  const { activeTab } = useSetting();
  const activeProjectId = useAppSelector(selectActiveProjectId);
  const globalValue = useAppSelector(selectCookieTrackingGlobal);
  const localValue = useAppSelector(selectCookieTrackingLocal);
  ...
  const applyingValue = useCheckApplyingSetting({
    ...
    globalValue,
    activeTab,
    defaultValue: DEFAULT_SETTINGS_REQUEST.cookieTracking,
  });
  ...
  const handleChange = (unfiltetedValue?: string) => {
    ...
    dispatch(
      updateSettingsRequest({
        cookieTracking:
          value as UpdateSettingsRequestInterface["cookieTracking"],
  ...
  return (
    <SettingItemHorizontalLayout className="items-center">
      <p className="flex-1">Cookie tracking</p>
      <div className="flex items-center gap-2">
        ...
        <SettingOptionSelector
          ...
          placeholder="Select Cookie Tracking"
...
export default SettingCookieTracking;
